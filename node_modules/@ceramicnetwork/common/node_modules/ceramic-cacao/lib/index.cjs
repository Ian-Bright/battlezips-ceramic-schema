...
class SiwxMessage {
  ...
  static fromCacao(cacao) {
    ...
    if (cacao.s) {
      ...
      if (cacao.s.t === 'eip191') siwx.type = exports.SignatureType.PERSONAL_SIGNATURE;
    }
...
function verifySolanaSignature(cacao, options) {
  ...
  const sigU8 = fromString.fromString(sig, 'base58btc');
  const issAddress = caip.AccountId.parse(cacao.p.iss.replace('did:pkh:', '')).address;
  const pubKeyU8 = fromString.fromString(issAddress, 'base58btc');

  if (!ed25519.verify(pubKeyU8, messageU8, sigU8)) {
    throw new Error(`Signature does not belong to issuer`);
  }
}
...
exports.SiwsMessage = SiwsMessage;
exports.SiwxMessage = SiwxMessage;
